...
const PUZZLES = [
  {
    id: 1,
    theme: "Let's Meet At The...",
    subtitle: "",
    emoji: "\uD83C\uDFDE\uFE0F",
    words: [
      { word: "PARK", hint: "A green public space perfect for laying out a blanket", revealedIndex: 0 },
      { word: "BEACH", hint: "A sandy shoreline where you can picnic by the water", revealedIndex: 2 },
      { word: "MEADOW", hint: "An open grassy field dotted with wildflowers", revealedIndex: 3 },
      { word: "GAZEBO", hint: "A small roofed structure offering shade in a garden or park", revealedIndex: 3 },
      { word: "PAVILION", hint: "A large open-sided shelter often found at parks for gatherings", revealedIndex: 4 },
    ],
    didYouKnow: "The most visited place for a picnic globally is Central Park in New York City, drawing an estimated 42 million visitors annually.",
  },
  {
    id: 2,
    theme: "Bring Your...",
    ...
    emoji: "\uD83E\uDDFA",
    words: [
      { word: "BASKET", hint: "A woven container for carrying your picnic food", revealedIndex: 0 },
      { word: "COOLER", hint: "A container that keeps drinks and food chilled outdoors", revealedIndex: 2 },
      { word: "THERMOS", hint: "An insulated container that keeps drinks hot or cold for hours", revealedIndex: 3 },
      { word: "BLANKET", hint: "The classic checkered spread you sit on for a picnic", revealedIndex: 0 },
      { word: "CUTLERY", hint: "Forks, knives, and spoons packed for your outdoor meal", revealedIndex: 4 },
    ],
    didYouKnow: "The classic red-and-white checkered picnic blanket pattern became popular because it was easy to spot from far away at outdoor sporting events and races.",
  },
  {
    id: 3,
    theme: "We Will Have Fun...",
    subtitle: "",
    emoji: "\uD83E\uDD4F",
    words: [
      { word: "KITE", hint: "A colorful toy flown on a string, carried by the wind", revealedIndex: 0 },
      { word: "CATCH", hint: "A simple game of tossing a ball back and forth", revealedIndex: 2 },
      { word: "PICKLE", hint: "a runner between bases while fielders throw the ball to tag them out", revealedIndex: 3 },
      { word: "FRISBEE", hint: "A flying disc tossed back and forth at the park", revealedIndex: 0 },
      { word: "SWIMMING", hint: "A cooling activity enjoyed at the beach or lake on a hot day", revealedIndex: 4 },
    ],
    didYouKnow: "College students in the 1940s tossed empty metal pie tins from the Frisbie Pie Company. Wham-O bought the idea and changed the spelling to \u201CFrisbee\u201D in 1957.",
  },
];

const GRAND_FACT = "Picnics took off in France after the French Revolution, when everyday people could finally visit the king's former parks and forests. Sturdy woven baskets were popular because they were light to carry and kept food safe on long walks, and wandering the grounds to admire flowers, birds, and bugs was just as much the point as the meal itself.";
...
const HowToPlayModal = ({ onClose }) => (
  <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={onClose}>
    <div className="bg-gradient-to-br from-green-950 to-slate-900 border border-red-500/30 rounded-xl p-5 max-w-sm w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="mt-5 text-center">
        <button onClick={onClose} className="px-6 py-2 rounded-full bg-gradient-to-r from-green-600 to-red-700 hover:from-green-500 hover:to-red-600 text-white font-medium text-sm transition-all" style={{ fontFamily: "Crimson Text, serif" }}>{"Let\u2019s Play"}</button>
      </div>
    </div>
  </div>
);
...
const ShareModal = ({ onClose }) => {
  ...
  const shareText = "\uD83E\uDDFA Letter Griddle Picnic\n\uD83E\uDD4F Completed all 3 puzzles!\n\uD83C\uDF3B\uD83C\uDF3B\uD83C\uDF3B\nPlay at lettergriddle.com/picnic";
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-gradient-to-br from-green-950 to-slate-900 border border-red-500/30 rounded-xl p-5 max-w-sm w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-amber-200 font-semibold text-lg" style={{ fontFamily: "Cormorant Garamond, serif" }}>Share Letter Griddle Picnic</h3>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-200 transition-colors text-lg w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-700/50">{"\u2715"}</button>
        </div>
        <div className="bg-slate-900/60 border border-red-500/20 rounded-lg p-4 mb-4 whitespace-pre-line text-slate-200 text-sm" style={{ fontFamily: "Georgia, serif" }}>
          {shareText}
        </div>
        <button onClick={handleCopy} className="w-full px-6 py-2.5 rounded-full bg-gradient-to-r from-green-600 to-red-700 hover:from-green-500 hover:to-red-600 text-white font-medium transition-all shadow-lg">
...
const EventFooter = () => {
  ...
  return (
    <div className="text-center mt-6 pb-4 space-y-1.5">
      <p className="text-slate-500 text-xs">A Letter Griddle Picnic</p>
...
const BURST_COLORS = ["#dc2626", "#16a34a", "#fbbf24", "#ffffff", "#4ade80", "#f87171"];
  ...
  return (
    <div
      ...
      className={`rounded-xl p-3 mb-3 border-2 transition-all duration-500 ${
        isCompleted ? "bg-gradient-to-br from-emerald-900/30 to-green-900/30 border-emerald-500/50"
          : celebrating ? "bg-gradient-to-br from-green-900/30 to-red-900/30 border-amber-400 scale-[1.02]"
          : isActive ? "bg-gradient-to-br from-slate-800/60 to-slate-900/60 border-green-500/40"
          : "bg-gradient-to-br from-slate-800/60 to-slate-900/60 border-slate-600/50 hover:border-green-500/30"
      }`}
    >
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setHintRevealed(!hintRevealed)} className={`text-xs px-3 py-1 rounded-full transition-all font-medium ${
            hintRevealed ? "bg-amber-500/20 text-amber-300 border border-amber-500/30" : "bg-gradient-to-r from-green-600 to-red-700 text-white hover:from-green-500 hover:to-red-600 shadow-sm"
      ...
      <div className="flex gap-1.5 justify-center flex-wrap mb-3">
        {word.split("").map((letter, idx) => {
          ...
          else if (isRevealed) slotClass += " bg-gradient-to-br from-yellow-800/40 to-yellow-900/40 border-yellow-400/60 text-yellow-200";
          else if (isCompleted) slotClass += " bg-gradient-to-br from-emerald-700/40 to-green-800/40 border-emerald-400/50 text-emerald-200 shadow-sm shadow-emerald-500/20";
          else if (isSelected) slotClass += " bg-green-500/20 border-green-400 text-green-100 ring-2 ring-green-400/50";
          else if (current) slotClass += " bg-slate-700/60 border-slate-500/60 text-slate-100 hover:border-green-400/50";
          else slotClass += " bg-slate-800/40 border-slate-600/40 text-transparent hover:border-green-500/40 hover:bg-slate-700/40";
          return (
          ...
          {bank.map((letter, idx) => {
            ...
            return (
              <div key={idx} onClick={() => handleBankClick(idx)} className={`w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center text-base sm:text-lg font-bold rounded-lg border-2 cursor-pointer select-none transition-all duration-200 ${
                isSelected ? "bg-green-500 border-green-300 text-white scale-110 shadow-lg shadow-green-500/40 ring-2 ring-green-300/50"
                  : "bg-gradient-to-br from-slate-600 to-slate-700 border-slate-500 text-slate-100 hover:from-green-600 hover:to-red-700 hover:border-green-400 hover:text-white hover:shadow-md"
...
export default function PicnicPage() {
  ...
  if (showIntro) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-green-950 via-slate-900 to-red-950 flex items-center justify-center p-4">
        <style>{`
          ...
          .shimmer-text { background: linear-gradient(90deg, #16a34a, #fde68a, #dc2626, #fde68a, #16a34a); background-size: 200% auto; -webkit-background-clip: text; -webkit-text-fill-color: transparent; animation: shimmer 5s linear infinite; }
        `}</style>
        <div className="max-w-lg w-full text-center">
          ...
          <h1 className="fade-up fade-up-2 text-3xl sm:text-4xl font-semibold mb-2 shimmer-text" style={{ fontFamily: "Cormorant Garamond, serif" }}>Letter Griddle Picnic</h1>
          <p className="fade-up fade-up-2 text-lg text-slate-300 mb-6 italic" style={{ fontFamily: "Crimson Text, serif" }}>A Summer Day in the Park</p>
          <p className="fade-up fade-up-3 text-slate-300 text-sm mb-2 leading-relaxed max-w-sm mx-auto" style={{ fontFamily: "Crimson Text, serif" }}>Three puzzles for a perfect summer picnic:   pick a spot, pack your basket, and get ready for some outdoor fun.</p>
          ...
          <div className="fade-up fade-up-5">
            <button onClick={() => setShowIntro(false)} className="px-8 py-3 rounded-full bg-gradient-to-r from-green-600 to-red-700 hover:from-green-500 hover:to-red-600 text-white font-semibold tracking-wide transition-all shadow-lg shadow-black/30 hover:shadow-black/40 hover:scale-105" style={{ fontFamily: "Crimson Text, serif", fontSize: "1.1rem" }}>Start the Picnic</button>
          </div>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-green-950 via-slate-900 to-red-950 flex items-center justify-center p-4">
        ...
        <div className="max-w-lg w-full text-center">
          ...
          <h2 className="fade-up fade-up-1 text-2xl sm:text-3xl font-semibold text-amber-200 mb-2" style={{ fontFamily: "Cormorant Garamond, serif" }}>Picnic Complete</h2>
          ...
          <p className="fade-up fade-up-2 text-slate-300 text-sm mb-6 italic" style={{ fontFamily: "Crimson Text, serif" }}>Thanks for playing Letter Griddle Picnic!</p>
          <div className="fade-up fade-up-2 mb-6">
            <button onClick={handleShareClick} className="px-6 py-2.5 rounded-full bg-gradient-to-r from-green-600 to-red-700 hover:from-green-500 hover:to-red-600 text-white font-medium transition-all shadow-lg hover:scale-105" style={{ fontFamily: "Crimson Text, serif" }}>
              Share Letter Griddle Picnic {"\uD83E\uDDFA"}
            </button>
          </div>
          ...
          <div className="fade-up fade-up-3 bg-gradient-to-br from-green-900/40 to-red-900/30 border-2 border-amber-500/30 rounded-xl p-5 mb-6 text-center">
          ...
          <div className="fade-up fade-up-4 bg-gradient-to-br from-slate-800/80 to-slate-900/80 rounded-xl border border-slate-700/50 p-5 mb-6">
            ...
            <div className="space-y-3">
              <a href="https://www.lettergriddle.com/play" target="_blank" rel="noopener noreferrer" className="block bg-gradient-to-r from-green-700/30 to-green-800/30 hover:from-green-600/40 hover:to-green-700/40 border border-green-600/30 rounded-lg p-3 transition-all group">
              ...
              <a href="https://www.lettergriddle.com" target="_blank" rel="noopener noreferrer" className="block bg-gradient-to-r from-red-700/30 to-red-800/30 hover:from-red-600/40 hover:to-red-700/40 border border-red-600/30 rounded-lg p-3 transition-all group">
                <div className="flex items-center gap-3">
                  <span className="text-xl">{"\u2615"}</span>
                  <div className="text-left">
  ...
  // GAME SCREEN
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-950 via-slate-900 to-red-950">
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Crimson+Text:ital,wght@0,400;0,600;0,700;1,400&family=Cormorant+Garamond:wght@300;400;500;600&display=swap');
      ...
      <div className="max-w-lg mx-auto px-3 py-4">
        <div className="text-center mb-4">
          <div className="flex items-center justify-between mb-1">
            <div className="w-8" />
            <span className="text-xs tracking-[0.2em] uppercase text-slate-300 font-medium">Letter Griddle {"\u2022"} Picnic</span>
          ...
          <h1 className="text-xl sm:text-2xl font-semibold text-amber-200 mt-1" style={{ fontFamily: "Cormorant Garamond, serif" }}>Letter Griddle Picnic</h1>
            ...
            {PUZZLES.map((p, idx) => (
              <button key={p.id} onClick={() => { if (puzzleComplete[idx] || idx === currentPuzzle) { setCurrentPuzzle(idx); setShowDidYouKnow(false); setActiveWordIdx(0); } }}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs transition-all ${
                  idx === currentPuzzle ? "bg-green-600/30 border border-green-500/40 text-amber-200"
        ...
        {showDidYouKnow && (
          <div className="fade-in mb-4">
            <div className="bg-gradient-to-br from-green-900/40 to-red-900/30 border-2 border-amber-500/30 rounded-xl p-5 text-center">
              <div className="text-2xl mb-2">{"\uD83C\uDF3B"}</div>
              <h3 className="text-amber-200 font-semibold text-lg mb-3" style={{ fontFamily: "Cormorant Garamond, serif" }}>Did You Know?</h3>
              ...
              {currentPuzzle < PUZZLES.length - 1 ? (
                <button onClick={goToNextPuzzle} className="px-6 py-2.5 rounded-full bg-gradient-to-r from-green-600 to-red-700 hover:from-green-500 hover:to-red-600 text-white font-medium transition-all shadow-lg hover:scale-105" style={{ fontFamily: "Crimson Text, serif" }}>
                  Continue to {PUZZLES[currentPuzzle + 1].theme} {PUZZLES[currentPuzzle + 1].emoji}
                </button>
              ) : (
                <button onClick={() => { setAllDone(true); setShowDidYouKnow(false); }} className="px-6 py-2.5 rounded-full bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-500 hover:to-green-500 text-white font-medium transition-all shadow-lg hover:scale-105" style={{ fontFamily: "Crimson Text, serif" }}>
                  Finish the Picnic {"\uD83E\uDDFA"}
                </button>
              )}
            </div>
