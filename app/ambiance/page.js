    ...
    <div className="min-h-screen relative overflow-hidden">
      <style>{`
        @keyframes gentleGlow {
          ...
          50% { text-shadow: 0 0 40px #fbbf24, 0 0 80px #fbbf24, 0 0 100px #fbbf24; }
        }
        .tagline-glow {
      ...
      {/* DAY background — warm sunrise orange */}
      <div className={`absolute inset-0 transition-opacity duration-1000 ${isEvening ? 'opacity-0' : 'opacity-100'}`}>
        <div className="absolute inset-0" style={{
          background: 'linear-gradient(135deg, #EA580C 0%, #F97316 30%, #FB923C 60%, #FDBA74 100%)',
        }} />
        <div className={`absolute inset-0 transition-opacity duration-1000 ${isEvening ? 'opacity-0' : 'opacity-100'}`}
          style={{ backgroundColor: 'rgba(255, 180, 100, 0.15)' }} />
      </div>

      {/* EVENING background — summer twilight: deep ocean blue to warm coral sunset */}
      <div className={`absolute inset-0 transition-opacity duration-1000 ${isEvening ? 'opacity-100' : 'opacity-0'}`}>
        <div className="absolute inset-0" style={{
          background: 'linear-gradient(160deg, #0c4a6e 0%, #1e3a8a 25%, #4c1d95 50%, #be185d 75%, #fb923c 100%)',
        }} />
        {/* Subtle warm glow at the horizon */}
        <div className="absolute inset-0" style={{
          background: 'radial-gradient(ellipse at 50% 100%, rgba(251,146,60,0.35) 0%, rgba(190,24,93,0.15) 40%, transparent 70%)',
        }} />
      </div>

      <div className={`absolute inset-0 transition-all duration-1000 ${isEvening ? 'bg-black/25' : 'bg-black/10'}`} />
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <div className="flex justify-between items-center p-4 md:p-6">
          ...
          <div className="flex items-center gap-3 px-4 py-2 rounded-full bg-black/30 backdrop-blur-sm border border-white/10">
            ...
            >
              <div className={`absolute top-1 w-4 h-4 rounded-full transition-all duration-500 bg-white shadow-lg ${isEvening ? 'left-7' : 'left-1'}`} />
            </button>
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4">
          ...
          <div className="mb-8 text-center">
            <h1
              className={`text-4xl md:text-6xl lg:text-7xl font-light tracking-[0.15em] transition-all duration-1000 ${isEvening ? 'text-sky-100' : 'text-amber-100'}`}
              style={{
                fontFamily: 'Georgia, serif',
                textShadow: isEvening
                  ? '0 0 40px rgba(186,230,253,0.3), 0 2px 10px rgba(0,0,0,0.5)'
            ...
            <p
              className={`mt-3 text-sm md:text-base tracking-[0.15em] transition-all duration-1000 ${isEvening ? 'text-sky-200/80' : 'text-amber-200/80'}`}
              style={{
                fontFamily: 'Georgia, serif',
          ...
          <p
            className={`text-center text-base md:text-lg ${isGlowing ? 'tagline-glow' : ''} max-w-md transition-all duration-1000 ${isEvening ? 'text-white/70' : 'text-white/80'}`}
            style={{
              fontFamily: 'Georgia, serif',
              fontStyle: 'italic',
          ...
          <div
            className="w-full max-w-sm p-5 rounded-2xl transition-all duration-500 bg-black/40 backdrop-blur-md border border-white/10 mt-8"
          ...
          >
            <div className="relative mb-4">
              ...
              >
                ...
                <ChevronDown className={`transition-transform text-white/50 ${showTrackMenu ? 'rotate-180' : ''}`} size={16} />
              </button>
                ...
                <div className="absolute top-full left-0 right-0 mt-2 rounded-xl shadow-2xl z-20 overflow-hidden bg-black/80 backdrop-blur-md border border-white/10">
                  {tracks.map((track, index) => (
                    <button
                      ...
                      className={`w-full px-4 py-3 text-left transition-all flex items-center gap-3 text-white/70 hover:bg-white/10 ${currentTrack === index ? 'bg-white/10' : ''}`}
                      style={{ fontFamily: 'Georgia, serif' }}
                    >
                      <span className="text-sm">{track.name}</span>
                      {currentTrack === index && <span className="ml-auto text-amber-400">●</span>}
                    </button>
                  ))}
                </div>
              )}
        ...
        <div className="p-4 md:p-6 text-center">
          <p className="text-xs text-white/40 tracking-wider" style={{ fontFamily: 'Georgia, serif', textShadow: '0 1px 5px rgba(0,0,0,0.5)' }}>
